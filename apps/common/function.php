...
// 获取打分按钮
function get_btn_score($idValue, $id = 'id', $btnName = '评分')
{
    $user_level = session('levels');
    if (! in_array('/' . M . '/' . C . '/score', $user_level) && session('id') != 1)
        return;
    $btn_html = "<a href='" . url("/" . M . '/' . C . "/score/$id/$idValue") . get_btn_qs() . "'  class='layui-btn layui-btn-xs'>$btnName</a>";
    return $btn_html;
}
// 获取修改按钮
